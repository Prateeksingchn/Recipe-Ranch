...
const RecipeBlog = () => {
    ...
    const fetchRecipeDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://api.edamam.com/search?r=http://www.edamam.com/ontologies/edamam.owl%23recipe_${id}&app_id=${APP_ID}&app_key=${APP_KEY}`
        );
  ...
  return (
    <div className="bg-[#F5F5F5] my-5 px-4 py-12 rounded-3xl">
      <Link
        to="/Recipes"
        className="text-green-600 hover:text-green-700 transition duration-300 mb-8 inline-block ml-[22.5%] "
      >
        <ArrowLeft className="inline mr-2" />
        Back to Recipes
      </Link>
      <article className="max-w-[800px] mx-auto bg-white rounded-lg shadow-lg shadow-slate-400 overflow-hidden">
          ...
          <div className="mt-8 mb-10">
            ...
            <div className="">
              ...
              <div className="bg-gray-50 p-6 rounded-lg">
                <h3 className="font-semibold mb-3 ml-3 text-[16px] text-gray-900">
                  Per serving:
                </h3>
                <ul className="grid grid-cols-2 gap-y-2">
                  {Object.entries(recipe.totalNutrients)
                    ...
                    .map(([key, nutrient]) => (
                      <li
                        key={key}
                        className="text-sm flex justify-between mr-4 ml-4 "
                      >
            ...
            {/* Health Labels */}
            <h2 className="text-2xl font-semibold mb-4 mt-5 text-gray-800">
              Health Labels
            </h2>
            <div className="flex flex-wrap">
              {recipe.healthLabels.map((label, index) => (
                <span
                  key={index}
                  className="bg-green-100 text-green-800 text-xs font-medium mr-2 mb-2 px-2.5 py-1 rounded-full"
                >
                  {label}
                </span>
              ))}
